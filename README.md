...
Важно: это утилита для настройки **прибора БалКом‑1А**, а не для «станка».

Справка по исходному прибору: https://oookin.ru/balkom1a.htm (в нашем проекте устройство дорабатывается).
...
- `192.168.4.1` обычно означает **AP режим**.
...
Положите 4 изображения в `docs/screenshots/` и переименуйте их так:

1. `docs/screenshots/01-main.png`
2. `docs/screenshots/02-wifi-scan.png`
3. `docs/screenshots/03-wifi-connect.png`
4. `docs/screenshots/04-firmware-upgrade.png`

После этого они будут отображаться прямо в README:
...
- Windows: в каталоге конфигурации пользователя (через `g_get_user_config_dir()`)

Также поддерживается «portable» режим: если `known_networks.ini` лежит рядом с исполняемым файлом, он будет использован.

## Для разработчиков

Инструкции по сборке и упаковке лежат в `docs/BUILD.md`.
# BalCom1A Configuration Utility
...
- Обновление прошивки устройства по UART через ROM bootloader ESP32‑S3 (кнопка **Firmware Upgrade**):
  - выбор файла прошивки (merged `.bin`/`.hex` или файл аргументов `flash_args` из сборки ESP‑IDF)
  - прогресс прошивки
  - примечание: во время прошивки ОС может временно считать приложение «не отвечает»; дождитесь завершения — операция продолжает выполняться
  - бэкап содержимого flash перед записью (в файл `*.backup.bin` рядом с выбранным файлом)
...
- Для прошивки (Firmware Upgrade) есть два варианта:
  - Рекомендуется (для пользователей): **бандлить flasher вместе с приложением**, тогда ничего устанавливать не нужно.
  - Для разработчиков: можно использовать `esptool` из `PATH`.

  Сборка с bundling:

  -DBALCOM_BUNDLE_FLASH_TOOL=ON
  -DBALCOM_FLASH_TOOL_EXECUTABLE=... (путь к flasher-утилите)

  Примечание: `esptool`, установленный через `pip`, обычно требует Python на машине пользователя.
  Для “полностью переносимой” поставки лучше использовать **standalone** flasher-бинарник.
...
Установка на «чистой» Ubuntu 22.04 LTS:

- Зависимости для запуска подтягиваются из **официальных репозиториев Ubuntu** (через `Depends` в `.deb`).
- На Ubuntu 22.04 пакеты GTK4 находятся в репозитории **universe**. Убедитесь, что он включён:
...
- Рекомендуемый способ установки локального `.deb` вместе с зависимостями:

```bash
sudo apt-get install -y ./balcom1a-wifi-configuration-utility_*.deb
```

Важно: если собирать `.deb` на более новой Ubuntu, он может потянуть версии библиотек, которых нет в 22.04. Для релиза лучше собирать пакет на Ubuntu 22.04.
...
- **Ubuntu: при установке .deb пишет, что пакет недоступен из репозитория**: обычно это означает, что не включён репозиторий `universe` (см. инструкции выше).
